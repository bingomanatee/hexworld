  ...
  class Hexworld {
    ...
    subdivide(subs) {
      ...
      console.log('subdividing by ', subs);
      this._zones = this._base.faces.map((face, index) => {
    ...
    _vertices() {
      const verts = _.map(this._base.vertices, (v) => v.clone());

      return verts;
    }

    _faces() {
      const faces = _.map(this._base.faces, (face) => face.clone());
    ...
    geometry() {
      ...
      let faces = this._faces();

      let geometry = _.extend(new THREE.Geometry(), {vertices, faces});
