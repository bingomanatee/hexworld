...
module.exports = class HexDivision {
  ...
  data(yell = false) {
    ...
    if (yell) console.log(table.toString());
    return table;
  }
  ...
  get boundary() {
    if (!this._boundry) this._boundry = this.tile.boundary.map(pToV);
   ...
   */
  childAt(n, w) {
    if (!this.validChildIndex(n, w)) return null;
    const k = _ci(n, w);
