...
class IsoTriangleEdge {
  ...
  get edgeIdentity() {
    if (!this._edgeIdentity) {
      ...
      const s = new Set([this.fromPointIndex, this.toPointIndex]);

      if (_.isEqual(s, new Set([this.triangle.rowPointIndex, this.triangle.colPointIndex]))) {
        this._edgeIdentity = IsoTriangleEdge.EDGE_ROW_COL;
      } else if (_.isEqual(s, new Set([
          this.triangle.rootPointIndex, this.triangle.colPointIndex
        ]))) {
